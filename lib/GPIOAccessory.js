...
constructor(log, accessory, wpi, onChar)
{
    ...
    switch(this.context.mode) {
        case "out":
            ...
            this.pinAction(this.inverted * 1, true);
            break;
        case "in":
            break;
        default:
...
GPIOAccessory.prototype.getOn = function(callback) {
    ...
    var on = ( this.inverted != this.wpi.digitalRead(this.context.sensepin) );
    callback(null, on);
}
...
GPIOAccessory.prototype.pinAction = function(action, sensor) {
    this.log('Turning ' + (action == (!this.inverted * 1) ? 'on' : 'off') + ' pin #' + this.context.pin);
    if (sensor) {
        this.wpi.digitalWrite(this.context.sensepin, action);
    }
...
GPIOAccessory.prototype.pinTimer = function() {
    ...
    setTimeout(function() {
        ...
        self.pinAction(self.inverted * 1, false);
